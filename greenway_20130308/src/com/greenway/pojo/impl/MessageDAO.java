 ...
 * 
 * @see com.greenway.pojo.impl.Message
...
public class MessageDAO extends HibernateDaoSupport implements IMessageDAO{
	...
	public Message findById(java.lang.Integer id) {
		...
		try {
			Message instance = (Message) getHibernateTemplate().get(
					"com.greenway.pojo.impl.Message", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Message> listMessageByUserId(User user){
		...
		List<Message> result = session.createQuery("from Message where user=:user order by date desc")
